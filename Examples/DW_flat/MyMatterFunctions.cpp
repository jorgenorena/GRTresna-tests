...
Real ScalarField::my_potential_function(const Real &phi_here) const
{
     ...
     * $$
    */
    returm 0.25*m_matter_params.lambda
        *(phi_here*phi_here - m_matter_params.v*m_matter_params.v);
        *(phi_here*phi_here - m_matter_params.v*m_matter_params.v);
}
...
Real ScalarField::my_phi_function(const RealVect &loc) const
{
    ...
    */
    ...
    return m_matter_params.v * tanh(sqrt(m_matter_params.lambda / 2.) * (rr - m_matter_params.R_o*L));
}
